...
:- spec_pre(tree_search/3,[tree(any),any,var]).
:- spec_post(tree_search/3,[tree(any),any,any],[tree(union(X)),compatible(X),list(compatible(X))]).
tree_search(T,Elem,Path) :-
    tree_search(T,Elem,[],Path).

:- spec_pre(tree_search/4,[tree(any),any,any,any]).
:- spec_post(tree_search/4,[tree(any),any,any,any],[tree(union(X)),compatible(X),list(compatible(X)),list(compatible(X))]).
