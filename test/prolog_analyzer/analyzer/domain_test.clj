...
(defn calculate-and-get-dom
  ...
  ([initial? term spec]
   (utils/get-dom-of-term (sut/fill-env-for-term-with-spec test-env term spec {:initial initial?}) term spec)))
...
(deftest fill-dom:var:var:filled
  (let [term (r/->VarTerm "X")
    ...
    (do-template [m]
                 ...
                 {:title        "Initial set to false"
                  :options      {:initial false}
                  :result       {(r/->VarTerm "X") (r/->ErrorSpec "Term cannot be var, because it is already nonvar")}}
...
(deftest fill-dom:var:tuple:filled
...
(deftest fill-dom:var:integer:empty
  (let [env test-env
        term (r/->VarTerm "X")
        spec (r/->IntegerSpec)]
    (do-template [m]
...
(deftest fill-dom:var:integer:empty
  (let [env (uber/add-nodes-with-attrs test-env [(r/->VarTerm "X") {:dom (r/->VarSpec)}])
    ...
    (do-template [m]
                 ...
                 {:title        "Initial set to false"
                  :options      {:initial false}
                  :result       {(r/->VarTerm "X")                                                                       (r/->ErrorSpec "No valid intersection of Var and Integer")
                                 }}
...
(deftest fill-dom:nonvar:var:empty
  (let [env test-env
    ...
    (do-template [m]
                 ...
                 {:title        "Initial set to true"
                  :options      {:initial true}
                  :result       {(r/->IntegerTerm 3)                                                                     (r/->IntegerSpec)
                                 }} ;; Why is the artificial term not a child of the compound?

                 {:title        "Initial set to false"
                  :options      {:initial false}
                  :result       {(r/->IntegerTerm 3)                                                                     (r/->ErrorSpec "Term cannot be var, because it is already nonvar")
                                 }}
...
(deftest bla
  (do-template [term dom new-type initial? overwrite? result]
               (is (= result (-> test-env
                                 (uber/add-nodes-with-attrs [term {:dom dom}])
                                 (sut/fill-env-for-term-with-spec term new-type {:initial initial? :overwrite overwrite?})
                                 (utils/get-dom-of-term term nil))))

               (r/->VarTerm "X")     (r/->VarSpec)        (r/->VarSpec)        true     false      (r/->VarSpec)
               (r/->VarTerm "X")     (r/->VarSpec)        (r/->IntegerSpec)    true     false      (r/->IntegerSpec)
               (r/->VarTerm "X")     (r/->IntegerSpec)    (r/->VarSpec)        true     false      (r/->IntegerSpec)
               (r/->VarTerm "X")     (r/->IntegerSpec)    (r/->IntegerSpec)    true     false      (r/->IntegerSpec)


               (r/->VarTerm "X")     (r/->VarSpec)        (r/->VarSpec)        false     false      (r/->VarSpec)
               (r/->VarTerm "X")     (r/->VarSpec)        (r/->IntegerSpec)    false     false      (r/DISJOINT (r/->VarSpec) (r/->IntegerSpec))
               (r/->VarTerm "X")     (r/->IntegerSpec)    (r/->VarSpec)        false     false      (sut/ALREADY-NONVAR)
               (r/->VarTerm "X")     (r/->IntegerSpec)    (r/->IntegerSpec)    false     false      (r/->IntegerSpec)


               (r/->VarTerm "X")     (r/->VarSpec)        (r/->VarSpec)        false     true       (r/->VarSpec)
               (r/->VarTerm "X")     (r/->VarSpec)        (r/->IntegerSpec)    false     true       (r/->IntegerSpec)
               (r/->VarTerm "X")     (r/->IntegerSpec)    (r/->VarSpec)        false     true       (sut/ALREADY-NONVAR)
               (r/->VarTerm "X")     (r/->IntegerSpec)    (r/->IntegerSpec)    false     true       (r/->IntegerSpec)

               (r/->IntegerTerm 3)   (r/->IntegerSpec)    (r/->VarSpec)        true     false      (r/->IntegerSpec)
               (r/->IntegerTerm 3)   (r/->IntegerSpec)    (r/->IntegerSpec)    true     false      (r/->IntegerSpec)


               (r/->IntegerTerm 3)   (r/->IntegerSpec)    (r/->VarSpec)        false     false      (sut/ALREADY-NONVAR)
               (r/->IntegerTerm 3)   (r/->IntegerSpec)    (r/->IntegerSpec)    false     false      (r/->IntegerSpec)


               (r/->IntegerTerm 3)   (r/->IntegerSpec)    (r/->VarSpec)        false     true       (sut/ALREADY-NONVAR)
               (r/->IntegerTerm 3)   (r/->IntegerSpec)    (r/->IntegerSpec)    false     true       (r/->IntegerSpec)
